    ...
    <div class="sidebar">
        <div class="widget">
            ...
            <ul class="widget_recent_post">
                @forelse (\Modules\Blog\Models\blog::where('status',1)->latest()->take(5)->get() as $recent)
                    <li>
                        <div wire:key="{{ $recent->id }}" class="post_footer">
                            <div class="post_img">
                                <a href="{{route('article.single.index', $recent->link )}}"><img
                                        src="/uploads/{{ $recent->img }}" alt="{{ $recent->title }}"></a>
                            </div>
                            <div class="post_content">
                                <h6>
                                    <a href="{{ route('article.single.index' , $recent->link )}}">{{ $recent->title }}</a>
                                </h6>
