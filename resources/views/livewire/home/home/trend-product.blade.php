        ...
        <div class="row">
            <div class="col-12">
                <div class="product_slider carousel_slider owl-carousel owl-theme dot_style1" data-loop="true"
                    ...
                    @forelse (\Modules\Product\Product\Models\product::where('status',1)->where('view','>=',1)->get() as $product)
                        @php

                            $gallery = \Modules\Product\Gallery\Models\gallery::where('product_id',$product->id)->first();
                                ...
                                <div class="product_img">
                                    <a href="{{ route('product.single.index', ['id'=>$product->id ,'link' =>$product->link])}}">
                                        <img src="/uploads/{{ $product->img}}" alt="{{ $product->title }}"
                                             class="resize_image">
                                        <img class="resize_image product_hover_img"
                                             src="/uploads/{{ $gallery->img ?? ''}}" alt="{{ $product->title }}">
                                    </a>
