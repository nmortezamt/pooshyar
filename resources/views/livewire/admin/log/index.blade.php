                        ...
                        @if ($readyToLoad)
                            ...
                            @foreach ($logs as $log)
                                ...
                                <tr role="row">
                                    <td>{{ $log->id }}</td>
                                    <td>
                                        @foreach (\Modules\User\Models\User::where('id' ,$log->user_id)->get() as $user)
                                            {{ $user->name }}
                                        @endforeach
                                    </td>
                                    <td>{{ $log->url }}</td>
                                    <td>
                                        @if ($log->actionType =='ایجاد')
                                            <span class="badge badge-success"
                                                  style="background-color: green">ایجاد</span>

                                        @elseif ($log->actionType == 'آپدیت')
                                            <span class="badge badge-warning"
                                                  style="background-color: rgb(16, 247, 255)">آپدیت</span>
                                        ...
                                        @elseif ($log->actionType =='بازیابی')
                                            <span class="badge badge-danger"
                                                  style="background-color: rgb(251, 185, 63)">بازیابی</span>
