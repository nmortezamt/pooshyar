@section('title','کامنت های محصولات')
<div>
    <div class="main-content" wire:init='loadComment'>

        <div class="tab__box">
            <div class="tab__items">
                <a class="tab__item" href="{{ route('comment.article.index') }}">کامنت های مقالات </a>
                    ...
                    <table class="table">
                        ...
                        <tbody>
                            ...
                            <tr role="row">
                                ...
                                <td>
                                    @if ($comment->user->img)
                                    <img src="/uploads/{{ $comment->user->img }}" alt="img" width="50" height="50">
                                        @else
                                        <p>تصویر ندارد</p>
                                    @endif
                                </td>
                                <td>{{ $comment->product->title }}</td>
                                <td>{{ $comment->user->name ?? $comment->user->number}}</td>
                                ...
                                <td>
                                    <button wire:click='remove({{ $comment->id }})' class="item-delete mlg-15"
                                        ...
                                        <a href="{{ route('comment.product.answer',$comment) }}" class="item-eye"
