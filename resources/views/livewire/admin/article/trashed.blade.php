                ...
                @if ($readyToLoad)
                    @forelse ($articles as $article)
                        <tr role="row" class="">
                            <td>{{ $article->id }}</td>
                            <td>
                                <img src="{{asset('uploads/'.$article->img )}}" alt="{{ $article->title }}"
                                     width="70px">
                            </td>
                            <td><a>{{ $article->title }}</a></td>
                            <td>
                                _دسته
                                @foreach (\Modules\Category\Models\category::where('id',$article->category_id)->get() as $category)
                                    {{ $category->title }}
                                @endforeach
                                <br>
                                _زیر دسته
                            ...
                            <td>
                                ...
                                <a wire:click='restorearticle({{ $article->id }})'
