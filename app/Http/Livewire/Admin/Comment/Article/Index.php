...
class Index extends Component
{
    ...
    public function updateCategorydisable($id)
    {
        ...
        log::create([
            ...
            'url' => 'غیر فعال کردن کامنت مقاله' . ' :' . $comment->name,
            'actionType' => 'غیر فعال'
        ]);
    }
    public function updateCategoryinable($id)
    {
        ...
        log::create([
            'user_id' => auth()->user()->id,
            'url' => 'فعال کردن کامنت مقاله' . " :" . $comment->name,
            'actionType' => 'فعال'
        ]);
    }
    public function remove($remove)
    {
        $comment = commentArticle::find($remove);
            $comment->delete();
            log::create([
                'user_id' => auth()->user()->id,
                'url' => 'حذف کامنت مقاله' . ' :' . $comment->name,
                'actionType' => 'حذف'
            ]);
    ...
    public function render()
    {
        ...
        $comments = $this->readyToLoad ? commentArticle::where(function($query){
        ...
        ->latest()->paginate(20) : [];
        return view('livewire.admin.comment.article.index',compact('comments'));
    }
}
